...
>
> 1.内存结构概述
>> JVM 为每一个线程分配一个内存区域，这个内存区域其实是虚拟的，实际上是寄存器、缓冲内存...，还会为成功程序分配一个主内存区域，每个主内存区域，存放共享变量
>>
> 2.关键词
>  
>> 可见性：指线程之间的共享变量，对于他们的操作是可见的，就是说线程1操作共享变量，线程2也同时要操作该共享变量，那么要等线程1将该共享变量的操作完成，并刷新到主内存时，线程2才会操作该共享变量。java中提供维护共享变量可见性的操作
>>
>> 原子性：指多个操作之间要么一起执行，要么都不执行，操作之间不可细化，不可分割
>>
>> 顺序性：指令之间执行的的顺序会按照编写代码的顺序执行
>
> 二、指令重排序
>
> 1.意义
...
>
> 2.分类
>> 编译器重排序：编译器在不改变单线程语义的前提下进行重排序
>>
>> 指令集并行重排序：在语句没有数据依赖的情况下，CPU采取指令并行重排序
>>
>> 内存系统重排序：在各级缓存以及内存中，CPU在加载和写入时，看上去是乱序执行的
>
> 3.JMM对指令重排序的解决方法
>>
>> 编译器级重排序，在一些特定的情况下，JMM禁止编译器的重排序，但不是所有
>>
>> JMM规则(happens-before原则)：
...
>>
>> 指令级重排序，JMM通过加入内存屏障来禁止指令级（cpu等硬件）的重排序
>
> 4.现代处理器对指令的重排序规则
>
>> 大多数现代处理器会对写-读操作进行重排序以优化程序效率，至于读-读、读-写、写-写各不相同，但是对于有数据依赖的两条语句，一定不会重排序
>> 这里的有数据依赖的语句必须在同一cpu中，如果个是多核处理器，采用了并行指令执行，有数据依赖的语句分别被两个cpu执行，那么他们的顺序与结果也是不可知的
>
> 三、内存屏障
>
> 1.内存屏障的分类
...
>> * StoreLoad屏障：对于这样的语句Store1; StoreLoad; Load2，在Load2及后续所有读取/写入操作执行前，保证Store1的写入对所有处理器可见
>
> 2.数据依赖性
>
>> 如果两个操作访问同一个变量，且这两个操作中有一个为写操作，此时这两个操作之间就存在数据依赖性。指令2的操作依赖指令1的操作结果
>
> 3.数据依赖性分类
>
>> 1. 指令1 写入，指令2 读取；
